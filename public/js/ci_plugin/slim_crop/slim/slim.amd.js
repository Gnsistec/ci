/*
 * Slim v1.0.7 - Image Cropping Made Easy
...
var ImageEditor = (function () {
    ...
    _createClass(ImageEditor, [{
        ...
        key: 'open',
        value: function open(image, ratio, callback) {
            var _this4 = this;
            ...
            this._preview.onload = function () {
                ...
                requestAnimationFrame(function () {
                    ...
                    var iw = _this4._preview.width;
                    var ih = _this4._preview.height;
                    var t;
                    var l;
                    var w = iw;
                    var h = ratio ? w * ratio : ih;
                    ...
                    _this4._cropper.resize(l, t, w, h);
...
var Slim = (function () {
    function Slim(element) {
        ...
        // auto accept mode for mime types
        this._autoAcceptCommonMimeTypes = false;

        // was an automatic input created
        this._autoInput = false;

        // was an automatic output created
        this._autoOutput = false;
    ...
    _createClass(Slim, [{
        ...
        value: function _init() {
            ...
            if (!this._output) {
                ...
                // if no service defined, we'll use this output element for posting data
                if (!this._options.service) {
                    this._element.appendChild(this._output);
                    this._autoOutput = true;
                }
            }
            ...
            var initialImageSrc = (this._getInitialImage() || {}).src;
            if (initialImageSrc) {
            ...
            if (this._isImageOnly()) {
                ...
                if (!this._input.hasAttribute('accept')) {
                    ...
                    this._autoAcceptCommonMimeTypes = true;
                } else {
        ...
        value: function _save() {
            ...
            this._options.onSave(data, function (data) {
                ...
                // if output field defined store for sync submit
                if (_this21._output) {
                    _this21._store(data);
                }
        ...
        value: function _upload(data, callback) {
            ...
            send(this._options.service, fd, function (loaded, total) {
                ...
                if (status === 'file-too-big') {
                    ...
                    html = _this22._options.statusInvalidResponse;
                } else {
...
(function () {
    ...
    var defaults = {
        ...
        // the errors that slim shows
        'statusFileSize': defaultLabel,
        'statusFileType': defaultLabel,
        'statusNoSupport': defaultLabel,
        'statusContentLength': passThrough,
        'statusInvalidResponse': passThrough,
        'statusUnknownResponse': passThrough,
        'statusUploadSuccess': passThrough,

        // the events to bind to
        'onInit': defaultFunction,
        'onTransform': defaultFunction,
    ...
    SlimButtons.concat(ImageEditorButtons).forEach(function (btn) {
        var capitalized = capitalizeFirstLetter(btn);
        defaults['button' + capitalized + 'ClassName'] = passThrough;
        defaults['button' + capitalized + 'Label'] = passThrough;
        defaults['button' + capitalized + 'Title'] = passThrough;
    });
